...
- `npm run shell:build` used Unix-only `cp`/`mkdir -p` shell commands, which don't work as invoked under Windows' PowerShell — switched to the cross-platform `shx` wrapper.
- The release workflow's three OS build jobs run in parallel, and each independently asked electron-builder to publish to GitHub — with no release for the tag yet exising, they raced to create their own, splitting one release's assets across two or three separate draft releases instead of one. A new job now creates the (draft) release for the tag before the platform matrix starts, so every platform finds and uploads to the same one. Windows is also now non-blocking (`continue-on-error`) so its still-being-worked-out platform issues don't hold up macOS/Linux, which are verified and working.
